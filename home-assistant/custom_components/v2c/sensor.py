...
from homeassistant.const import CONF_HOST, UnitOfEnergy, UnitOfPower, UnitOfTime, UnitOfElectricCurrent
...
TRYDAN_SENSOR_TYPES: tuple[SensorEntityDescriptionV2C, ...] = (
    ...
    SensorEntityDescriptionV2C(
        ...
        device_class=SensorDeviceClass.POWER,
        value="0",
        func=lambda self: "Inaccesible or unavailable" if self.charger.getContractedPower() == -1.0 else self.charger.getContractedPower(),
    ),
)
...
class SensorV2C(SensorEntity):
    ...
    def send_lock(self):
        ...
        try:
            ...
            _LOGGER.warning("Could not update status for Lock", )
    ...
    def send_data(self):
        """It send the data the users choose and wrote."""
